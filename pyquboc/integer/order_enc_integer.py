...
from pyquboc import Placeholder, Constraint, SubH
from pyquboc.array import Array
from pyquboc.integer import IntegerWithPenalty
...
class OrderEncInteger(IntegerWithPenalty):
    ...
    def __init__(self, label, value_range, strength):
        ...
        self.array = Array.create(
            label, shape=self._num_variables, vartype='BINARY')
        ...
        for i in range(self._num_variables - 1):
            ...
            self.constraint += Constraint(b - a * b,
                                          const_label, condition=lambda x: x == 0)
    ...
    def more_than(self, k):
        ...
        assert k > lower, "This value is always equal to or more than {}".format(
            k)
