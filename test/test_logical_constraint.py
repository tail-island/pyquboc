...
from pyquboc import Binary, AndConst, OrConst, XorConst, NotConst
...
class TestLogicalConstraint(unittest.TestCase):
    ...
    def test_and(self):
        ...
        model = exp.compile()
        self.assertTrue(model.energy(
            {"a": 1, "b": 1, "c": 1}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 0, "c": 0}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 1, "c": 0}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 0, "c": 0}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 1, "c": 0}, vartype="BINARY") > 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 0, "c": 1}, vartype="BINARY") > 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 0, "c": 1}, vartype="BINARY") > 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 1, "c": 1}, vartype="BINARY") > 0)
    ...
    def test_xor(self):
        ...
        model = exp.compile()
        self.assertTrue(model.energy(
            {"a": 1, "b": 1, "c": 0, "aux_xor": 1}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 0, "c": 1, "aux_xor": 0}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 1, "c": 1, "aux_xor": 0}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 1, "c": 0, "aux_xor": 1}, vartype="BINARY") == 0)
        self.assertTrue(model.energy(
            {"a": 0, "b": 0, "c": 1, "aux_xor": 1}, vartype="BINARY") > 0)
        self.assertTrue(model.energy(
            {"a": 1, "b": 1, "c": 1, "aux_xor": 1}, vartype="BINARY") > 0)
