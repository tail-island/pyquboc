...
from pyquboc import Binary, Placeholder, Array, SubH, Constraint, assert_qubo_equal
...
class TestModel(unittest.TestCase):
    ...
    def test_decode_sample(self):
        ...
        # self.assertRaises(TypeError, lambda: model.decode_sample(
        #     (1, 1), vartype="BINARY"))
    ...
    def test_higher_order(self):
        ...
        sample = {'x[0]': 1, 'x[1]': 1, 'x[2]': 1,
                  'x[3]': 1, 'x[0] * x[1]': 1, 'x[2] * x[3]': 1}
        e = model.energy(sample, vartype='BINARY')

        sample = {'x[0]': 0, 'x[1]': 1, 'x[2]': 1,
                  'x[3]': 1, 'x[0] * x[1]': 0, 'x[2] * x[3]': 1}
        e = model.energy(sample, vartype='BINARY')
